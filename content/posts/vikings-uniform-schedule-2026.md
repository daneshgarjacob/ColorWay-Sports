---
title: "Vikings 2026 Uniform Schedule: When They Wear Classic, Rivalries, and the Winter Warrior Whiteout"
category: "NFL"
homepageFeature: true
date: "2026-07-29"
updatedDate: "2026-08-27"
excerpt: "The Vikings confirmed every 2026 home uniform: Classic throwbacks against Green Bay, the new Rivalries jersey against Detroit, and the Winter Warrior whiteout."
gradient: "linear-gradient(135deg, #4F2683 0%, #1a0d2e 55%, #FFC62F 130%)"
logoSrc: "/logos/leagues/nfl.png"
logoSrc2: "/logos/teams/nfl-minnesota-vikings.png"
coverImage: "/images/posts/vikings-uniform-schedule-2026/cover.jpg"
coverImagePosition: "center center"
league: "nfl"
teams: ["minnesota-vikings"]
---

<img src="/images/posts/vikings-uniform-schedule-2026/cover.jpg" alt="Minnesota Vikings 2026 uniform schedule graphics showing the Week 15 Rivalries jersey vs the Lions and the Week 16 Winter Whiteout vs the Commanders" style="width: 100%; border-radius: 10px; margin-bottom: 2.5em;" />
<p style="text-align: center; font-size: 0.75em; color: #999; margin-top: -2em; margin-bottom: 2.5em;">via the Minnesota Vikings</p>

Most teams let their uniform news trickle out one week at a time. The Vikings did not. Minnesota published its 2026 game themes with the jersey attached to every single home date, which means the U.S. Bank Stadium half of this schedule is confirmed rather than projected. Three of the nine are genuine uniform changes: the **Classic** throwback opens the season against Green Bay on the 25th anniversary of 9/11, the brand-new **Rivalries** jersey debuts against Detroit on Sunday Night Football in Week 15, and the **Winter Warrior** whiteout returns against Washington in Week 16. Add the all-purple combo against the Colts and the purple-over-white primary for the rest, and here is the full Vikings 2026 uniform schedule, week by week.

<div style="display: flex; justify-content: center; margin: 2em 0;">
  <blockquote class="twitter-tweet">
    <a href="https://twitter.com/Vikings/status/2082481542001988082"></a>
  </blockquote>
</div>

<div style="margin: 2em 0; font-family: Inter, -apple-system, BlinkMacSystemFont, sans-serif;">
<div style="background: #f5f7fa; border: 1px solid #e3e6ec; border-radius: 12px; padding: 16px 18px; display: flex; align-items: center; gap: 14px; flex-wrap: wrap;">
<div style="width: 44px; height: 44px; border-radius: 10px; background: #f0e6ff; display: flex; align-items: center; justify-content: center; flex-shrink: 0;"><svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#4F2683" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20.38 3.46 16 2a4 4 0 0 1-8 0L3.62 3.46a2 2 0 0 0-1.34 2.23l.58 3.47a1 1 0 0 0 .99.84H6v10c0 1.1.9 2 2 2h8a2 2 0 0 0 2-2V10h2.15a1 1 0 0 0 .99-.84l.58-3.47a2 2 0 0 0-1.34-2.23z"/></svg></div>
<div style="flex: 1 1 160px; min-width: 0;"><div style="color: #2a1245; font-size: 15px; font-weight: 700; line-height: 1.3;">Shop Vikings Jerseys</div><div style="color: #6b7180; font-size: 13px; line-height: 1.4;">Classic, Winter Warrior, and more, from Fanatics</div></div>
<a href="https://fanatics.93n6tx.net/5kZn3j?u=https%3A%2F%2Fwww.fanatics.com%2Fsearch%3Fquery%3Dminnesota%2520vikings%2520jersey" target="_blank" rel="sponsored noopener" data-fanatics-jersey-cta style="background: #4F2683; color: #ffffff; font-size: 14px; font-weight: 700; padding: 10px 18px; border-radius: 8px; text-decoration: none; white-space: nowrap; margin-left: auto;">Shop now →</a>
</div>
<p style="font-size: 12px; color: #9aa0ac; margin: 8px 2px 0;">ColorWay Sports may earn a commission on purchases, at no extra cost to you.</p>
</div>

## The Confirmed Vikings Home Uniform Schedule

All nine home games, straight from the team. This is the part nobody has to guess at.

<div style="margin: 1.5em 0; padding: 1.5em 1.5em; background: linear-gradient(135deg, #4F2683 0%, #6a35ad 100%); border-radius: 16px; border: 2px solid #FFC62F;">
  <p style="font-size: 1em; color: #ffffff; margin: 0 0 0.9em; text-transform: uppercase; letter-spacing: 2px; font-weight: 800; text-align: center;">Every Home Game, Confirmed</p>
  <p style="font-size: 1em; color: #ffffff; margin: 0.5em 0; line-height: 1.5;"><strong style="color:#FFC62F;">Classic</strong> &middot; Week 1 vs. Packers &middot; Sunday, September 13</p>
  <p style="font-size: 1em; color: #ffffff; margin: 0.5em 0; line-height: 1.5;"><strong style="color:#ffffff;">Purple over White</strong> &middot; Week 4 vs. Dolphins &middot; Sunday, October 4</p>
  <p style="font-size: 1em; color: #ffffff; margin: 0.5em 0; line-height: 1.5;"><strong style="color:#FFC62F;">All-Purple</strong> &middot; Week 7 vs. Colts &middot; Sunday, October 25</p>
  <p style="font-size: 1em; color: #ffffff; margin: 0.5em 0; line-height: 1.5;"><strong style="color:#ffffff;">Purple over White</strong> &middot; Week 9 vs. Bills &middot; Monday, November 9</p>
  <p style="font-size: 1em; color: #ffffff; margin: 0.5em 0; line-height: 1.5;"><strong style="color:#ffffff;">Purple over White</strong> &middot; Week 12 vs. Falcons &middot; Sunday, November 29</p>
  <p style="font-size: 1em; color: #ffffff; margin: 0.5em 0; line-height: 1.5;"><strong style="color:#ffffff;">Purple over White</strong> &middot; Week 13 vs. Panthers &middot; Sunday, December 6</p>
  <p style="font-size: 1em; color: #ffffff; margin: 0.5em 0; line-height: 1.5;"><strong style="color:#FFC62F;">Rivalries</strong> &middot; Week 15 vs. Lions &middot; Sunday, December 20</p>
  <p style="font-size: 1em; color: #ffffff; margin: 0.5em 0; line-height: 1.5;"><strong style="color:#FFC62F;">Winter Warrior</strong> &middot; Week 16 vs. Commanders &middot; Date TBD</p>
  <p style="font-size: 1em; color: #ffffff; margin: 0.5em 0; line-height: 1.5;"><strong style="color:#ffffff;">Purple over White</strong> &middot; Week 18 vs. Bears &middot; Date TBD</p>
</div>

## The Full 2026 Vikings Uniform Schedule, Week by Week

Home games carry the confirmed jersey. Road games follow the white road set, which is the standard away look. Week 6 is the bye.

<div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(138px, 1fr)); gap: 10px; margin: 1.5em 0;">
  <div style="background: #46216f; color: #ffffff; border: 2px solid #FFC62F; border-radius: 10px; padding: 12px 10px; text-align: center;"><div style="font-size: 0.68em; font-weight: 700; letter-spacing: 1px; opacity: 0.85;">WEEK 1</div><div style="font-size: 0.95em; font-weight: 800; margin: 4px 0;">vs Packers</div><div style="font-size: 0.76em; font-weight: 800; text-transform: uppercase; letter-spacing: 0.5px; color: #FFC62F;">★ Classic</div></div>
  <div style="background: #f1f3f8; color: #333; border: 1px solid #dfe3ea; border-radius: 10px; padding: 12px 10px; text-align: center;"><div style="font-size: 0.68em; font-weight: 700; letter-spacing: 1px; opacity: 0.7;">WEEK 2</div><div style="font-size: 0.95em; font-weight: 800; margin: 4px 0;">at Bears</div><div style="font-size: 0.76em; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">White</div></div>
  <div style="background: #f1f3f8; color: #333; border: 1px solid #dfe3ea; border-radius: 10px; padding: 12px 10px; text-align: center;"><div style="font-size: 0.68em; font-weight: 700; letter-spacing: 1px; opacity: 0.7;">WEEK 3</div><div style="font-size: 0.95em; font-weight: 800; margin: 4px 0;">at Buccaneers</div><div style="font-size: 0.76em; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">White</div></div>
  <div style="background: #4F2683; color: #ffffff; border-radius: 10px; padding: 12px 10px; text-align: center;"><div style="font-size: 0.68em; font-weight: 700; letter-spacing: 1px; opacity: 0.85;">WEEK 4</div><div style="font-size: 0.95em; font-weight: 800; margin: 4px 0;">vs Dolphins</div><div style="font-size: 0.76em; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">★ Purple / White</div></div>
  <div style="background: #f1f3f8; color: #333; border: 1px solid #dfe3ea; border-radius: 10px; padding: 12px 10px; text-align: center;"><div style="font-size: 0.68em; font-weight: 700; letter-spacing: 1px; opacity: 0.7;">WEEK 5</div><div style="font-size: 0.95em; font-weight: 800; margin: 4px 0;">at Saints</div><div style="font-size: 0.76em; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">White</div></div>
  <div style="background: #eceff3; color: #98a0ac; border: 1px dashed #cdd3db; border-radius: 10px; padding: 12px 10px; text-align: center;"><div style="font-size: 0.68em; font-weight: 700; letter-spacing: 1px; opacity: 0.8;">WEEK 6</div><div style="font-size: 0.95em; font-weight: 800; margin: 4px 0;">Bye</div><div style="font-size: 0.76em; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">Off</div></div>
  <div style="background: #35155c; color: #ffffff; border-radius: 10px; padding: 12px 10px; text-align: center;"><div style="font-size: 0.68em; font-weight: 700; letter-spacing: 1px; opacity: 0.85;">WEEK 7</div><div style="font-size: 0.95em; font-weight: 800; margin: 4px 0;">vs Colts</div><div style="font-size: 0.76em; font-weight: 800; text-transform: uppercase; letter-spacing: 0.5px; color: #FFC62F;">★ All-Purple</div></div>
  <div style="background: #f1f3f8; color: #333; border: 1px solid #dfe3ea; border-radius: 10px; padding: 12px 10px; text-align: center;"><div style="font-size: 0.68em; font-weight: 700; letter-spacing: 1px; opacity: 0.7;">WEEK 8</div><div style="font-size: 0.95em; font-weight: 800; margin: 4px 0;">at Lions</div><div style="font-size: 0.76em; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">White</div></div>
  <div style="background: #4F2683; color: #ffffff; border-radius: 10px; padding: 12px 10px; text-align: center;"><div style="font-size: 0.68em; font-weight: 700; letter-spacing: 1px; opacity: 0.85;">WEEK 9</div><div style="font-size: 0.95em; font-weight: 800; margin: 4px 0;">vs Bills</div><div style="font-size: 0.76em; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">★ Purple / White</div></div>
  <div style="background: #f1f3f8; color: #333; border: 1px solid #dfe3ea; border-radius: 10px; padding: 12px 10px; text-align: center;"><div style="font-size: 0.68em; font-weight: 700; letter-spacing: 1px; opacity: 0.7;">WEEK 10</div><div style="font-size: 0.95em; font-weight: 800; margin: 4px 0;">at Packers</div><div style="font-size: 0.76em; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">White</div></div>
  <div style="background: #f1f3f8; color: #333; border: 1px solid #dfe3ea; border-radius: 10px; padding: 12px 10px; text-align: center;"><div style="font-size: 0.68em; font-weight: 700; letter-spacing: 1px; opacity: 0.7;">WEEK 11</div><div style="font-size: 0.95em; font-weight: 800; margin: 4px 0;">at 49ers (MEX)</div><div style="font-size: 0.76em; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">White</div></div>
  <div style="background: #4F2683; color: #ffffff; border-radius: 10px; padding: 12px 10px; text-align: center;"><div style="font-size: 0.68em; font-weight: 700; letter-spacing: 1px; opacity: 0.85;">WEEK 12</div><div style="font-size: 0.95em; font-weight: 800; margin: 4px 0;">vs Falcons</div><div style="font-size: 0.76em; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">★ Purple / White</div></div>
  <div style="background: #4F2683; color: #ffffff; border-radius: 10px; padding: 12px 10px; text-align: center;"><div style="font-size: 0.68em; font-weight: 700; letter-spacing: 1px; opacity: 0.85;">WEEK 13</div><div style="font-size: 0.95em; font-weight: 800; margin: 4px 0;">vs Panthers</div><div style="font-size: 0.76em; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">★ Purple / White</div></div>
  <div style="background: #f1f3f8; color: #333; border: 1px solid #dfe3ea; border-radius: 10px; padding: 12px 10px; text-align: center;"><div style="font-size: 0.68em; font-weight: 700; letter-spacing: 1px; opacity: 0.7;">WEEK 14</div><div style="font-size: 0.95em; font-weight: 800; margin: 4px 0;">at Patriots</div><div style="font-size: 0.76em; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">White</div></div>
  <div style="background: #1f1030; color: #ffffff; border: 2px solid #FFC62F; border-radius: 10px; padding: 12px 10px; text-align: center;"><div style="font-size: 0.68em; font-weight: 700; letter-spacing: 1px; opacity: 0.85;">WEEK 15</div><div style="font-size: 0.95em; font-weight: 800; margin: 4px 0;">vs Lions</div><div style="font-size: 0.76em; font-weight: 800; text-transform: uppercase; letter-spacing: 0.5px; color: #FFC62F;">★ Rivalries</div></div>
  <div style="background: #ffffff; color: #4F2683; border: 2px solid #4F2683; border-radius: 10px; padding: 12px 10px; text-align: center;"><div style="font-size: 0.68em; font-weight: 700; letter-spacing: 1px; opacity: 0.7;">WEEK 16</div><div style="font-size: 0.95em; font-weight: 800; margin: 4px 0;">vs Commanders</div><div style="font-size: 0.76em; font-weight: 800; text-transform: uppercase; letter-spacing: 0.5px;">★ Winter Warrior</div></div>
  <div style="background: #f1f3f8; color: #333; border: 1px solid #dfe3ea; border-radius: 10px; padding: 12px 10px; text-align: center;"><div style="font-size: 0.68em; font-weight: 700; letter-spacing: 1px; opacity: 0.7;">WEEK 17</div><div style="font-size: 0.95em; font-weight: 800; margin: 4px 0;">at Jets</div><div style="font-size: 0.76em; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">White</div></div>
  <div style="background: #4F2683; color: #ffffff; border-radius: 10px; padding: 12px 10px; text-align: center;"><div style="font-size: 0.68em; font-weight: 700; letter-spacing: 1px; opacity: 0.85;">WEEK 18</div><div style="font-size: 0.95em; font-weight: 800; margin: 4px 0;">vs Bears</div><div style="font-size: 0.76em; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">★ Purple / White</div></div>
</div>

<p style="font-size: 0.85em; color: #777; margin: 0 0 2.5em;">What the grid can and cannot promise: every ★ is <strong>officially confirmed by the Vikings</strong>, and that covers all nine home games at U.S. Bank Stadium. The eight road games are the white road set, which is the standard away look but has not been announced week by week, and that includes Week 11 in Mexico City, where San Francisco is the designated home team. Pant colors on the road are an equipment-staff call the week of each game. The Week 16 and Week 18 dates are still open because the league holds late-season windows for flexible scheduling. The Rivalries uniform was revealed on August 25.</p>

## Classic Throwback (Week 1, Confirmed)

The season opens in the throwbacks. Minnesota will wear its **Classic** uniform against the Green Bay Packers on Sunday, September 13 at 3:25 PM, a game the team is building around the 25th anniversary of 9/11. The Classic set is the deeper, richer purple jersey with oversized slab-serif numbers, gold trim, the old sleeve stripes, and the vintage horn helmet with a gray facemask, worn over white pants with a gold stripe flanked by purple.

This is the right uniform for the occasion and the right occasion for the uniform. The Classic is the best thing hanging in Minnesota's closet, a straight line back to the Purple People Eaters era without any of the modern clutter, and putting it on the field for the opener against Green Bay means the oldest rivalry in the division gets the oldest look in the building. We grade the Classic an **A-**, and the gray facemask is the detail that does the heavy lifting. Modern Vikings helmets use a purple cage that flattens the front of the shell. The gray reads like a photograph from 1975.

<a href="https://fanatics.93n6tx.net/5kZn3j?u=https%3A%2F%2Fwww.fanatics.com%2Fsearch%3Fquery%3Dminnesota%2520vikings%2520throwback%2520jersey" target="_blank" rel="sponsored noopener" data-fanatics-jersey-cta style="display: inline-block; margin: 0.5em 0 0.25em; background: #2f6bed; color: #ffffff; font-size: 13px; font-weight: 700; padding: 8px 16px; border-radius: 8px; text-decoration: none;">Shop the Classic Throwback on Fanatics →</a>

## Week 15 vs. Lions: The "Rivalries" Uniform, Revealed

<img src="/images/posts/vikings-uniform-schedule-2026/rivalries-group.jpg" alt="Minnesota Vikings Rivalries uniform reveal, players in the deep purple jersey with ivory and gold rune-carved numbers and Nordic knot trim" style="width: 100%; border-radius: 10px; margin-bottom: 0.5em;" />
<p style="text-align: center; font-size: 0.75em; color: #999; margin: 0 0 2em;">via the Minnesota Vikings</p>

The design landed with the rest of the 2026 class on August 25, and it goes all the way into Norse territory. The base is a deep "Purple Dynasty" shade paired with ivory and metallic gold, the two-tone numbers are cut to look chiseled from stone like runes, and a new chiseled VIKINGS wordmark matches them. A Nordic knot pattern drawn from the Gjallarhorn at U.S. Bank Stadium wraps the shoulders and runs down the pants, coming to a point meant to symbolize a sword. The matte purple helmet keeps the ivory horn, adds a gold knot stripe and a gold face mask, and puts SKOL on the front bumper transcribed into actual Viking Age rune characters, with the bumper's point mimicking the nose piece on a historical Viking helmet.

Our verdict: this is four separate ideas arguing with each other on top of the best base uniform in the division, and it loses on every count that matters. Start with the color. Minnesota's regular purple is vivid, saturated, and instantly readable on television; the "Purple Dynasty" shade that replaces it here is deeper and duller, and under stadium light it drifts toward a flat, muddy plum that takes the team's single greatest asset and turns the contrast down.

The gold face mask is the fatal decision. Minnesota's helmet works because the white horn and the light cage frame the front of the shell and keep it sharp, and swapping in gold closes that face down and pushes the whole thing toward costume. Behind it, the design will not stop talking: Nordic knot borders on the shoulders, the same knots running the pants, chiseled rune-carved numbers, a new chiseled wordmark, and a rune inscription on the bumper. Each of those could have carried a uniform. Together they compete, and nothing gets room to land.

The rune bumper is the clearest example of a detail built for a launch graphic rather than a football field. SKOL transcribed into Viking Age characters is a genuinely interesting piece of research, and it is also an alphabet essentially nobody in the stadium reads, which means the most prominent piece of type on the helmet communicates nothing to the people looking at it. That is the kind of idea that feels clever in August and tired by the second wearing.

<div style="margin: 2em 0; padding: 1.5em 2em; background: linear-gradient(135deg, #4F2683 0%, #1f1030 100%); border-radius: 16px; box-shadow: 0 4px 20px rgba(79, 38, 131, 0.25); text-align: center;">
  <p style="margin: 0 0 6px; font-size: 0.7em; font-weight: 700; color: rgba(255,255,255,0.75); text-transform: uppercase; letter-spacing: 3px;">ColorWay Sports Grade</p>
  <p style="margin: 0; font-size: 2.75em; font-weight: 900; color: #ffffff; line-height: 1; letter-spacing: -1px;">F</p>
  <p style="font-size: 0.9em; color: #ffffff; margin: 0.9em 0 0; letter-spacing: 1px;">Four Ideas Fighting · The Gold Cage Is the Fatal One</p>
</div>

The frustrating part is how little Minnesota needed this. The Classic throwback earlier on this same schedule is an A- and the standard purple set is among the best in football, so the Vikings walked into the program holding a stronger hand than almost anyone in the class and spent it on Norse set dressing. The rune numbers are the one piece worth saving.

<img src="/images/posts/vikings-uniform-schedule-2026/rivalries-helmet-runes.jpg" alt="Vikings Rivalries matte purple helmet with gold face mask, gold knot stripe, and SKOL in rune characters on the bumper" style="width: 100%; border-radius: 10px; margin: 1.5em 0 0.5em;" />
<p style="text-align: center; font-size: 0.75em; color: #999; margin: 0 0 2em;">via the Minnesota Vikings</p>

<img src="/images/posts/vikings-uniform-schedule-2026/rivalries-knot-pattern.jpg" alt="Vikings Rivalries Nordic knot pattern on the shoulders and pants, inspired by the Gjallarhorn" style="width: 100%; border-radius: 10px; margin-bottom: 0.5em;" />
<p style="text-align: center; font-size: 0.75em; color: #999; margin: 0 0 2em;">via the Minnesota Vikings</p>

The debut slot is still as good as it gets: **Sunday, December 20 at 7:20 PM on Sunday Night Football against the Detroit Lions**, which is also the team's Walter Payton NFL Man of the Year game. A prime-time NFC North game against the exact rival the set was built around, and the uniform stays in the rotation for three seasons. For the wider context, see our [explainer on the NFL Rivalries uniform program](/stories/what-is-the-nfl-rivalries-uniform-program).


Rivalries jerseys and the rest of the collection go on sale **Tuesday, September 1**, at NFLShop.com, Nike, Fanatics, team stores and select retail locations.

<a href="https://fanatics.93n6tx.net/5kZn3j?u=https%3A%2F%2Fwww.fanatics.com%2Fsearch%3Fquery%3Dminnesota%2520vikings%2520rivalries%2520jersey" target="_blank" rel="sponsored noopener" data-fanatics-jersey-cta style="display: inline-block; margin: 0.5em 0 0.25em; background: #2f6bed; color: #ffffff; font-size: 13px; font-weight: 700; padding: 8px 16px; border-radius: 8px; text-decoration: none;">Shop the Rivalries jersey on Fanatics →</a>

## Week 16 vs. Commanders: The Winter Warrior "Whiteout"

<div style="display: flex; justify-content: center; margin: 1.5em 0;"><img src="/images/posts/vikings-uniform-schedule-2026/week-16-winter-whiteout.jpg" alt="Minnesota Vikings Week 16 Winter Whiteout Winter Warrior uniform announcement vs the Washington Commanders, date and time to be determined" style="width: 100%; max-width: 420px; border-radius: 12px;" /></div>
<p style="text-align: center; font-size: 0.75em; color: #999; margin-top: -0.5em;">via the Minnesota Vikings</p>

One quick bit of housekeeping, because the graphic can be read two ways: "Winter Whiteout" is the name of the game-day theme, and the jersey the team wears for it is officially the **Winter Warrior**. It is Minnesota's all-white alternate, white jersey and white pants accented with metallic gray and purple, topped by the first all-white helmet in franchise history, and it is back for a third straight season. The date and time are still to be determined, but the opponent is set: the **Washington Commanders** in Week 16, which doubles as the Inspire Change game.

We are big fans of this one. The Winter Warrior leans all the way into what Minnesota actually is, a cold-weather team that plays a December brand of football, and an all-white kit under the U.S. Bank Stadium lights with the metallic-gray trim reads like frost on the sleeves. A stadium-wide whiteout in the crowd to match the uniform is exactly the kind of coordinated moment that makes a look feel like an event instead of a jersey change. We grade the Winter Warrior a **B+**, and the purple horns on that white shell are the detail that keeps it from feeling generic.

Here is the Winter Warrior itself, so you can see exactly what the whiteout looks like, the white helmet and all:

<div style="display: flex; justify-content: center; margin: 2em 0;">
  <blockquote class="twitter-tweet" data-dnt="true"><a href="https://twitter.com/Vikings/status/1798701377209630911"></a></blockquote>
</div>

<a href="https://fanatics.93n6tx.net/5kZn3j?u=https%3A%2F%2Fwww.fanatics.com%2Fsearch%3Fquery%3Dminnesota%2520vikings%2520winter%2520warrior%2520jersey" target="_blank" rel="sponsored noopener" data-fanatics-jersey-cta style="display: inline-block; margin: 0.5em 0 0.25em; background: #2f6bed; color: #ffffff; font-size: 13px; font-weight: 700; padding: 8px 16px; border-radius: 8px; text-decoration: none;">Shop the Winter Warrior whiteout on Fanatics →</a>

## All-Purple (Week 7, Confirmed)

The boldest thing the Vikings do with their standard set gets a date. Purple jersey over purple pants comes out for the Indianapolis Colts on Sunday, October 25 at noon, and the team listed it as a straight uniform choice rather than a themed promotion. Monochrome purple is a genuinely divisive look, and we land on the side that likes it: the Vikings own one of the two or three best base colors in the league, and the mono treatment is the only version of the uniform where the color gets to be the entire statement. The white pants are more classical. The purple pants are more Minnesota.

<a href="https://fanatics.93n6tx.net/5kZn3j?u=https%3A%2F%2Fwww.fanatics.com%2Fsearch%3Fquery%3Dminnesota%2520vikings%2520purple%2520jersey" target="_blank" rel="sponsored noopener" data-fanatics-jersey-cta style="display: inline-block; margin: 0.5em 0 0.25em; background: #2f6bed; color: #ffffff; font-size: 13px; font-weight: 700; padding: 8px 16px; border-radius: 8px; text-decoration: none;">Shop the purple jersey on Fanatics →</a>

## Purple Over White (5 Home Games, Confirmed)

The primary home look, purple jersey over white pants, carries the remaining five home dates, and it draws most of the schedule's biggest nights: Crucial Catch and Latino Heritage against the Miami Dolphins in Week 4, then Salute to Service against the Buffalo Bills on Monday, November 9, which is also when **Adrian Peterson** is enshrined as the 29th member of the Vikings Ring of Honor. My Cause My Cleats against the Atlanta Falcons follows in Week 12, the Carolina Panthers in Week 13, and the Chicago Bears close the regular season at home in Week 18.

Worth being clear about what those themes are and are not. Crucial Catch, Salute to Service, My Cause My Cleats, and Inspire Change are league initiatives expressed through cleats, captain patches, decals, and towels. Only three of them change the jersey in Minnesota this year, and the Vikings' own schedule spells out which.

## The White Road Set (8 Games, Expected)

The white road jersey handles all eight away games: at the Bears, Buccaneers, Saints, Lions, Packers, 49ers, Patriots, and Jets. The Vikings have not published a road-by-road jersey schedule, so these follow the standard NFL convention where the visiting team wears white. Pants are the variable, since Minnesota alternates between purple and white below the waist depending on the week.

Two road dates stand out. Week 11 is the Mexico City game against the San Francisco 49ers at Estadio Azteca on Sunday night, November 22, where San Francisco is the designated home team and Minnesota is the visitor despite the neutral site. And Week 8 at Detroit on November 1 is the first of two meetings with the Lions, which means the Rivalries jersey is being saved for the rematch in Minneapolis, exactly as the program is designed.

<a href="https://fanatics.93n6tx.net/5kZn3j?u=https%3A%2F%2Fwww.fanatics.com%2Fsearch%3Fquery%3Dminnesota%2520vikings%2520white%2520jersey" target="_blank" rel="sponsored noopener" data-fanatics-jersey-cta style="display: inline-block; margin: 0.5em 0 0.25em; background: #2f6bed; color: #ffffff; font-size: 13px; font-weight: 700; padding: 8px 16px; border-radius: 8px; text-decoration: none;">Shop the white road jersey on Fanatics →</a>

## How the Vikings Rotation Works in 2026

The logic is clean. The Classic throwback opens the season on a date that deserves a piece of history. The all-purple combo gets one midseason showcase. The two December alternates take the two December windows most likely to be nationally televised, the Rivalries debut on Sunday night and the Winter Warrior in Christmas week. Everything else is purple over white, which is the look people picture when they picture the Vikings. Four distinct home appearances out of nine games is more variety than most teams manage without any of it feeling like a costume change.

## The Bottom Line

The Vikings did the thing more teams should do: they published a jersey for every home game instead of teasing one at a time. Three real uniform changes anchor the slate, the Classic against Green Bay in Week 1, the new Rivalries jersey against Detroit in Week 15, and the Winter Warrior whiteout against Washington in Week 16, with the all-purple combo against the Colts as the fourth look worth circling. The rest is the purple-and-white primary paired with the league's usual game-day causes. The Rivalries design itself arrived August 25 and it is the one miss on an otherwise excellent slate: deep purple with rune-carved numbers, Nordic knot trim, and a gold face mask, which we grade an **F**. Minnesota's own Classic throwback in Week 1 is an A- and shows exactly what this franchise can do when it trusts the purple instead of decorating it.

## Frequently Asked Questions

**What jersey are the Vikings wearing today?**

Minnesota confirmed its home schedule and it is the busiest in the league, so check the week rather than assuming purple. Week 1 is the Classic throwback, Week 7 is the all-purple set, Week 15 against Detroit is the new Rivalries jersey, and Week 16 against Washington is the Winter Warrior whiteout. The other five home games are purple over white, and every road game is the white set.

**What jerseys are the Vikings wearing this week?**

Minnesota confirmed its home schedule and it is the busiest in the league, so check the week rather than assuming purple. Week 1 is the Classic throwback, Week 7 is the all-purple set, Week 15 against Detroit is the new Rivalries jersey, and Week 16 against Washington is the Winter Warrior whiteout. The other five home games are purple over white, and every road game is the white set.

**What uniform did the Vikings wear last week?**

For the uniform we logged in their most recent game, see the [NFL uniform tracker](/stories/nfl-uniform-tracker-2026), which we update through the week. The grid above is the season plan, and the tracker is the record of what actually went on the field.

**What are the Vikings wearing next week?**

Check the grid above. Five different home looks are confirmed for 2026, so the week decides it. Road games are always white.

**What uniform do the Vikings wear against the Packers in 2026?**

The Classic throwback. Minnesota confirmed it will wear its Classic uniform, the deeper purple jersey with slab-serif numbers, gold trim, and the vintage horn helmet with a gray facemask, for the Week 1 home opener against Green Bay on Sunday, September 13, a game built around the 25th anniversary of 9/11.

**When do the Vikings wear their Rivalries uniform in 2026?**

The Vikings debut their new NFL x Nike Rivalries jersey in Week 15 against the Detroit Lions on Sunday, December 20 at 7:20 PM, as part of Sunday Night Football. The uniform was revealed on August 25: deep purple with ivory and metallic gold, rune-carved two-tone numbers, a Nordic knot pattern from the Gjallarhorn, and a matte purple helmet with a gold face mask and SKOL in rune characters on the bumper. It stays in the rotation for three seasons.

**What is the Vikings' Winter Warrior uniform, and when do they wear it in 2026?**

The Winter Warrior is Minnesota's all-white alternate, a white jersey and white pants accented with metallic gray and purple, worn with the first all-white helmet in franchise history. In 2026 it returns for the Week 16 "Winter Whiteout" home game against the Washington Commanders. The date and time are to be determined.

**Is "Winter Whiteout" the same as the Winter Warrior jersey?**

Essentially, yes. "Winter Whiteout" is the name of the Week 16 game-day theme, and the Winter Warrior is the all-white uniform the Vikings wear for it, so you will see both names attached to the same game.

**When do the Vikings wear all-purple in 2026?**

Once, at home in Week 7 against the Indianapolis Colts on Sunday, October 25. Purple jersey over purple pants is the mono version of the standard home set, and the Vikings listed it on their own uniform schedule.

**How many uniforms do the Vikings have in 2026?**

Five looks in the rotation: the purple home primary, the white road set, the Classic throwback, the new Rivalries jersey, and the Winter Warrior alternate. The all-purple combo is the home primary worn with purple pants rather than a separate uniform.

**Is the full Vikings 2026 uniform schedule official?**

The home half is. The Vikings confirmed the jersey for all nine home games when they announced their 2026 game themes. The eight road games follow the standard white road convention and are our projection until the team says otherwise.

<div data-division-crosslinks style="margin: 2.25em 0; padding: 1.35em 1.5em; background: #f6f7f9; border: 1px solid #e3e7ec; border-radius: 12px;"><p style="font-size: 0.7em; font-weight: 800; letter-spacing: 2.2px; text-transform: uppercase; color: #5f7085; margin: 0 0 0.35em;">The rest of the NFC North</p><p style="font-size: 0.9em; color: #5f7085; margin: 0 0 0.9em; line-height: 1.5;">Every division rival's jersey calendar for 2026.</p><a href="/stories/bears-uniform-schedule-2026" style="display: block; padding: 9px 0; border-bottom: 1px solid #e3e7ec; color: #14284b; font-weight: 700; text-decoration: none;">Chicago Bears 2026 Uniform Schedule</a><a href="/stories/lions-uniform-schedule-2026" style="display: block; padding: 9px 0; border-bottom: 1px solid #e3e7ec; color: #14284b; font-weight: 700; text-decoration: none;">Detroit Lions 2026 Uniform Schedule</a><a href="/stories/packers-uniform-schedule-2026" style="display: block; padding: 9px 0; border-bottom: 1px solid #e3e7ec; color: #14284b; font-weight: 700; text-decoration: none;">Green Bay Packers 2026 Uniform Schedule</a><a href="/stories/nfl-uniform-schedule-2026" style="display: block; margin-top: 12px; padding: 11px 14px; background: #14284b; border-radius: 8px; color: #ffffff; font-weight: 800; font-size: 13.5px; text-decoration: none; text-align: center; letter-spacing: 0.3px;">All 32 teams &middot; the 2026 NFL Uniform Schedule &rarr;</a></div>

## More NFL Coverage

- [NFL Uniform Tracker 2026](/stories/nfl-uniform-tracker-2026) — what all 32 teams wear, game by game
- [What Is the NFL Rivalries Uniform Program?](/stories/what-is-the-nfl-rivalries-uniform-program) — how the division-rival jerseys came to exist
- [Green Bay Packers 2026 Uniform Schedule](/stories/packers-uniform-schedule-2026) — the Week 1 and Week 10 opponent
- [Buffalo Bills 2026 Uniform Schedule](/stories/bills-uniform-schedule-2026) — the Week 9 opponent, also fully confirmed
- [NFL Uniform Release Dates 2026](/stories/nfl-uniform-release-dates-2026) — every team's new-look reveal, tracked

*Home uniform assignments and game themes confirmed by the Minnesota Vikings; road-jersey projections follow the standard home-and-away rotation. Schedule via team and league sources. ColorWay Sports is an independent design site and is not affiliated with, endorsed by, or sponsored by the Minnesota Vikings or the National Football League. Uniform images via the Minnesota Vikings.*
