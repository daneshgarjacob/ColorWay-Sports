...
export default async function StoryPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // Same idea for football: NFL schedule posts open with the helmet, jersey and
  // pants from the club's last played game, read off the NFL uniform tracker.
  const nflTeam =
    post.league === "nfl" && /-uniform-schedule-2026$/.test(slug)
      ? buildNflTeamIndex().find((t) => t.scheduleSlug === slug)
      : undefined;
  const nflTracker = nflTeam ? await getPostBySlug("nfl-uniform-tracker-2026") : null;
  const nflLatest =
