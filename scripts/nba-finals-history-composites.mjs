...
const LOGOS_DIR = resolve(__dirname, "../public/images/posts/nba-finals-logo-history");
...
const GRADIENT_FROM = "#FFF4D6";
const GRADIENT_TO = "#E6D9B0";
...
async function buildCoverComposite({ output }) {
  const topRow = [1986, 1996, 2004, 2008];
  const bottomRow = [2014, 2018, 2025];
  const cellSize = 240;
...
(async () => {
  console.log("=== NBA Finals Logo History composite builder ===\n");
  ...
  await buildCoverComposite({
    output: `${LOGOS_DIR}/nba-finals-logo-history-cover.png`,
  });
  ...
  await buildGridComposite({
    ...
    padding: 50,
    yearFontSize: 26,
    output: `${LOGOS_DIR}/nba-finals-logo-history-grid.png`,
    watermarkBand: 130,
  });
