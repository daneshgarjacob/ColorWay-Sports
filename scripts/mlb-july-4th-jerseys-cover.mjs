#!/usr/bin/env node
// ColorWay Sports composite cover: 2026 MLB Fourth of July jerseys showcase.
...
const OUT = resolve(__dirname, "../public/images/posts/mlb-july-4th-jerseys-cover.jpg");
const W = 1600, H = 900;
...
<svg width="${W}" height="${H}" xmlns="http://www.w3.org/2000/svg">
  ...
  <text x="${W / 2}" y="510" text-anchor="middle" class="title" font-size="98">MLB'S JULY 4TH JERSEYS</text>
  <text x="${W / 2}" y="588" text-anchor="middle" class="t2" font-size="44">EVERY 2026 STARS &amp; STRIPES SET</text>
  <text x="${W / 2}" y="654" text-anchor="middle" class="sub" font-size="23">ALL 30 TEAMS, REVIEWED</text>
</svg>`;

let img = await sharp(Buffer.from(bgSvg))
  .composite([{ input: Buffer.from(textSvg), top: 0, left: 0 }]).png().toBuffer();

// MLB logo, top-center, if available and valid
